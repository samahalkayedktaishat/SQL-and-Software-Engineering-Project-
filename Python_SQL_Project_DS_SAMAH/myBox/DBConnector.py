...
class DBConnector(ABC):
    ...
    def __init__(self: object, dbserver: str, dbname: str, dbusername: str,                  dbpassword: str, DSN: str = None, trustedmode: bool =  False, viewname: str = "", isPasswordObfuscated:bool = True):    
        try:
            ...
            if(self._dbConduit is None):
                ...
                try:
                    ...
                    if('windows' in identifiedOS.lower()):
                        ...
                        if(self.dbDSN is not None and self.dbDSN != ''):
                                self._m_conduit =                                     pyodbc.connect('DSN=' + self.dbDSN +                                     ';UID=' + self.dbUserName +                                     ';PWD=' + self._obfuscator.deObfuscate(self._dbUserPassword) + ';')

                        if(self.dbIsTrustedMode == False and (self.dbDSN is None or dbDSN == '')):
                            self._m_conduit =                                 pyodbc.connect('DRIVER=' + self.selectedDriver + ';SERVER=' + self.dbServer +                                 ';DATABASE=' + self.dbName + ';UID=' + self.dbUserName +                                 ';PWD=' + self._obfuscator.deObfuscate(self._dbUserPassword) + ';')
                        else:
                            print(self.selectedDriver,self.dbServer,self.dbName)
                            self._m_conduit =                                 pyodbc.connect('DRIVER=' + self.selectedDriver + ';SERVER=' + self.dbServer +                                 ';DATABASE=' + self.dbName + ';Trusted_Connection=yes;')
    ...
    def ExecuteQuery_withRS(self, query: str)-> pd.DataFrame:
